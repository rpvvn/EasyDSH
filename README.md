...
DSH（DeepSeek Harness）一键启动器 —— 一个单文件、绿色便携的 Windows GUI 小工具，用于启动、重启、停止 DSH Web 服务，省去每次手动输入 `npx @deepseek-ai/dsh web` 的麻烦。

基于 **WPF**（.NET Framework 4.0）构建，矢量渲染界面，无需安装任何第三方依赖，双击即用。
...
1. 下载 [Release](../../releases) 中的 `DSH-Launcher.exe`，或按下方说明自行编译
2. 双击运行 `DSH-Launcher.exe`
3. 首次使用点「**安装 DSH**」（或确保 DSH 已通过 `npx` 缓存 / 全局安装）
4. 点「**一键启动**」，等服务就绪会自动打开浏览器访问 `http://127.0.0.1:3080`
...
| 动作 | 实现 |
|------|------|
| 启动 | 优先使用全局 `dsh web`，否则回退到 `npx --yes @deepseek-ai/dsh web` |
| 停止 | WMI 扫描命令行含 `deepseek-ai` 的 node 进程 + `netstat` 找监听 3080 的进程，`taskkill /F /T` 强制结束 |
| 检测 | `node -v` / `npm -v` / `npm config get prefix|cache` / 端口 3080 连通性 |
| 端口 | 默认 `127.0.0.1:3080`（与 DSH Web GUI 一致） |

## 📁 目录结构

```
├── DshLauncher.cs   # 全部源码（单文件，纯代码构建 UI，无 XAML）
├── icon.ico         # 应用图标
├── build.bat        # 一键编译脚本
├── README.md
└── .gitignore
...
- **点「停止服务」后浏览器仍能打开**：属正常现象，浏览器页面刷新后即失效；如需彻底关闭请在浏览器手动关闭标签页。
- **图标不显示**：请确认编译时 `icon.ico` 与源码在同一目录。
